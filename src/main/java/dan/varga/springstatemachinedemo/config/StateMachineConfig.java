package dan.varga.springstatemachinedemo.config;

import dan.varga.springstatemachinedemo.config.actions.AuthAction;
import dan.varga.springstatemachinedemo.config.actions.PreAuthAction;
import dan.varga.springstatemachinedemo.config.guards.PaymentIdGuard;
...
// The component will be responsible for generating State Machines.
...
public class StateMachineConfig extends StateMachineConfigurerAdapter<PaymentState, PaymentEvent> {
    ...
    /*
     Because these property names matching the class names, Spring will be able to autowire them, even with the
     generic declarations, without qualifiers.
    */
    ...
    public void configure(StateMachineStateConfigurer<PaymentState, PaymentEvent> states) throws Exception {
        states.withStates()
                // Initial state will be 'NEW'.
                .initial(PaymentState.NEW)
                // Load the remaining states. These are all terminal states.
    ...
    public void configure(StateMachineTransitionConfigurer<PaymentState, PaymentEvent> transitions) throws Exception {
        // Set up 3 events for 3 outcomes for Pre-Auth.
...
//    // Ensure that the header is not null. If it is, the action will be prevented.
...
//                        .build());
//            } else {
//                System.out.println("Auth Declined! Transaction Error!");
